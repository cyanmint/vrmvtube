...
func update_platform_info(
	platform_name: String, vcam_supported: bool, gdmp_available: bool
) -> void:
	...
	var vcam_text := (
		" (Virtual Camera: Supported)" if vcam_supported else " (Virtual Camera: Not Supported)"
	)
	var tracking_text := ""
...
func update_webcam_status(
	status: String, is_active: bool = false, gdmp_available: bool = false
) -> void:
...
func _on_position_x_changed(value: float) -> void:
	...
	var pos := Vector3(
		value,
		position_y_slider.value if position_y_slider else 0.0,
		position_z_slider.value if position_z_slider else 0.0
	)
...
func _on_position_y_changed(value: float) -> void:
	...
	var pos := Vector3(
		position_x_slider.value if position_x_slider else 0.0,
		value,
		position_z_slider.value if position_z_slider else 0.0
	)
	model_position_changed.emit(pos)
...
func _on_rotation_x_changed(value: float) -> void:
	...
	var rot := Vector3(
		deg_to_rad(value),
		deg_to_rad(rotation_y_slider.value if rotation_y_slider else 0.0),
		deg_to_rad(rotation_z_slider.value if rotation_z_slider else 0.0)
	)
...
func _on_rotation_y_changed(value: float) -> void:
	...
	var rot := Vector3(
		deg_to_rad(rotation_x_slider.value if rotation_x_slider else 0.0),
		deg_to_rad(value),
		deg_to_rad(rotation_z_slider.value if rotation_z_slider else 0.0)
	)
	model_rotation_changed.emit(rot)
