...
func _process(_delta: float) -> void:
	...
	for key in current_blend_shapes.keys():
		current_blend_shapes[key] = lerp(
			current_blend_shapes[key], target_blend_shapes[key], smoothing_factor
		)

	# Smooth head rotation
	current_head_rotation = current_head_rotation.lerp(target_head_rotation, smoothing_factor)
